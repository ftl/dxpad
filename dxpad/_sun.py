...
def _ecliptic_position(days):
    ...
    ecliptic_lon = lon + 1.915 * math.sin(deg_to_rad(g)) + 0.02 * math.sin(deg_to_rad(2 * g))
    r = 1.00014 - 0.01671 * math.cos(deg_to_rad(g)) - 0.0014 * math.cos(deg_to_rad(2 * g))
    return (ecliptic_lon, r)

def _ecliptic_obliquity(days):
    centuries = days / 36525.0
    return 23.43929111 - centuries * (46.836769 / 3600.0 - centuries * (0.0001831 / 3600.0 + centuries * (0.00200340 / 3600.0 - centuries * (0.576e-6 / 3600.0 - centuries * 4.34e-8 / 3600.0))))

def _equitorial_position(ecliptic_lon, ecliptic_obliquity):
    alpha = rad_to_deg(math.atan(math.cos(deg_to_rad(ecliptic_obliquity)) * math.tan(deg_to_rad(ecliptic_lon))))
    delta = rad_to_deg(math.asin(math.sin(deg_to_rad(ecliptic_obliquity)) * math.sin(deg_to_rad(ecliptic_lon))))
...
def _latitude(angle, equitorial_position):
    return -rad_to_deg(math.atan(-math.cos(deg_to_rad(angle)) / math.tan(deg_to_rad(equitorial_position[1]))))
...
def calculate_day_night_terminator(time):
    ...
    equitorial_position = _equitorial_position(ecliptic_position[0], ecliptic_obliquity)
